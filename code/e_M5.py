# is_http를 if_factor에 포함하기
# 5번 : [sample] urlparse한 컬럼 별 TF-IDF + 파생변수 + LGBM_cv5 (임계값 결정) + class_weight='balanced'
# 파생변수 : is_capital , is_factor(is_http 포함) , count_sc , is_http , count_path, len_query
# auc-roc = 0.951941 , 최대 f1-score = 0.8501718, g-mean = 0.8811666 , 최대 g-mean = 0.8978998 (임계값 0.7)
# Fit time: 338.200 seconds
import pandas as pd
import numpy as np
...
df_raw['is_factor'] = df_raw['is_factor']+df_raw['is_http']
test_raw['is_factor'] = test_raw['is_factor']+test_raw['is_http']

df_raw[(df_raw['is_factor']==False)&(df_raw['is_http']==True)]
test_raw[(test_raw['is_factor']==False)&(test_raw['is_http']==True)]



df_tfidf = df_raw[['scheme', 'username', 'password', 'hostname', 'port', 'path', 'query','fragment']]
test_tfidf = test_raw[['scheme', 'username', 'password', 'hostname', 'port', 'path', 'query','fragment']]


# 1. TF-IDF
segment = ['scheme', 'username', 'password', 'hostname', 'port', 'path', 'query','fragment']
tfidf_vectorizers = {}
train_tfidf_matrices = {}
test_tfidf_matrices = {}

for i in segment:
    tfidf_vectorizers[i] = TfidfVectorizer(analyzer='char', ngram_range=(2, 5), max_features=50000)
    train_tfidf_matrices[i] = tfidf_vectorizers[i].fit_transform(df_tfidf[i].fillna(''))
    test_tfidf_matrices[i] = tfidf_vectorizers[i].transform(test_tfidf[i].fillna(''))
...
df_X_combined = hstack([ *[train_tfidf_matrices[i] for i in segment], csr_matrix(df_X_features)]).tocsr()
test_X_combined = hstack([ *[test_tfidf_matrices[i] for i in segment], csr_matrix(test_X_features)]).tocsr()
...
feature_names = []
for i in segment:
    feature_names.extend(tfidf_vectorizers[i].get_feature_names_out().tolist())
...
# test_pred.to_csv('../data/test/e_M5_pred.csv', index=False)
