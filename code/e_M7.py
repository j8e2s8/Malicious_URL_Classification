# 7번 : [sample] (전체 url, urlparse한 컬럼 별) TF-IDF + 파생변수 + LGBM_cv5 (임계값 결정) + class_weight='balanced'
# 파생변수 : is_capital , is_factor(is_http 포함) , count_sc , is_http , count_path, len_query
# auc-roc = 0.9497126 , 최대 f1-score = 0.8446104, g-mean = 0.8891653999999999 , 최대 g-mean = 0.89443 (임계값 0.6)
# Fit time: 
import pandas as pd
import numpy as np
...
for i in segment:
    tfidf_vectorizers[i] = TfidfVectorizer(analyzer='char', ngram_range=(2, 5), max_features=5000)
