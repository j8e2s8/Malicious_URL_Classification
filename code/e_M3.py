# 3번 : [sample] 전체 url TF-IDF + 파생변수 + LGBM_cv5 (임계값 결정) + class_weight='balanced'
# 파생변수 : is_capital , is_factor , count_sc , is_http
# auc-roc = 0.9471888  (임계값 0.6)
import pandas as pd
import numpy as np
...
# 대문자 여부 파생변수
df['is_capital'] = df['URL'].str.contains(r'[A-Z]', regex=True)

# 악성일 가능성이 높은 요소들의 True False 파생변수
# . or - or _ 로 IP 스타일, 인코딩, 포트, 공백, 영어 외 언어, 유효한 특수문자 (! , @ , # , % , $ , & , * , ( , ) , _ , + , - , = , [ , ] , ' , \ , : , " , , , / , ? , `)
df['is_factor'] = df['URL'].str.contains(r'\b\d{1,3}(\.\d{1,3}){3}\b|\b\d{1,3}(\-\d{1,3}){3}\b|\b\d{1,3}(\_\d{1,3}){3}\b', regex=True) \
						| df['URL'].str.contains(r'[^a-zA-Z0-9\n\t\^{};|<>~\.]', regex=True )


# 특수문자 수 파생변수
df['count_sc'] = df['URL'].str.count(r'[!@#$%&*()_+\-=\[\]\'\\:",?`]')


# http 여부 파생변수
df['is_http'] = df['URL'].str.contains(r"http(?!s)", case=False, regex=True)
...
# 2. 파생변수 준비
df_features = df[['is_capital', 'is_factor', 'count_sc', 'is_http']] 
df_X_features = df_features.astype('float32').values  

test_features = test[['is_capital', 'is_factor', 'count_sc', 'is_http']] 
test_X_features = test_features.astype('float32').values 

# 3. TF-IDF(희소행렬) + 파생변수(밀집행렬) 결합 
df_X_combined = hstack([df_X_tfidf, df_X_features]).tocsr()    # dtype : object가 있으면 에러남.
test_X_combined = hstack([test_X_tfidf, test_X_features]).tocsr()  # .tocsr() <- 이걸 해야 .split 했을 때 인덱싱을 할 수 있음
...
print(f"임계값 결정 : {threshold_optimal}") # 0.6
print(f"valid_auc_mean : {valid_auc_mean}") # [0.9471888]
...
feature_names = vectorizer.get_feature_names_out().tolist() + list(df_features.columns)
...
# test_pred.to_csv('../data/test/e_M3_pred.csv', index=False)
